...
export const defaultHomeSettings: HomeSettings = {
  sections: {
    hero: { enabled: true, order: 1, background: "white" },
    about: { enabled: true, order: 2, background: "white" },
    objectives: { enabled: true, order: 3, background: "slate" },
    latestUpdates: { enabled: true, order: 4, background: "white" },
    gallery: { enabled: true, order: 5, background: "slate" },
    pressConferences: { enabled: true, order: 6, background: "white" },
    executiveCommittee: { enabled: true, order: 7, background: "slate" },
    officeBearers: { enabled: true, order: 8, background: "white" },
    membership: { enabled: true, order: 9, background: "slate" },
  },

  hero: {
    eyebrow: "Greater Noida Press Club",
    identityLabel: "Journalism • Media • Community",
    title: "Connecting Journalism, Media & Community",
    description:
    ...
    quickLinks: [
      ...
      { label: "Press Conferences", href: "/press-conference" },
  ...
  about: {
    ...
    features: [],
    showStats: true,
    statsLabels: ["Members", "Press Release", "Active Events"],
    media: [],
  },
  ...
  membership: {
    eyebrow: "Membership",
    title: "Become a Part of the Press Club",
    description:
...
export function mergeHomeSettings(
  ...
  >(
    ...
    ({
      ...
      media: normalizeMedia(
        (incoming as Record<string, unknown> | undefined)?.media
      ),
    }) as T;
  ...
  return {
    ...
    sections: Object.fromEntries(
      Object.entries(defaultHomeSettings.sections).map(
        ([key, fallback]) => [
          key,
          {
            ...fallback,
            ...(source.sections?.[key as HomeSectionKey] || {}),
          },
        ]
      )
    ...
    hero: {
      ...
      ...(source.hero || {}),
      quickLinks: Array.isArray(source.hero?.quickLinks)
        ? source.hero!.quickLinks
        : defaultHomeSettings.hero.quickLinks,
    },
    ...
    objectives: {
      ...
      ...(source.objectives || {}),
      cards: Array.isArray(source.objectives?.cards)
        ? source.objectives.cards
        : defaultHomeSettings.objectives.cards,
      media: normalizeMedia(source.objectives?.media),
    },

    latestUpdates: mergeSection(
      defaultHomeSettings.latestUpdates,
      source.latestUpdates
    ),

    gallery: mergeSection(
      defaultHomeSettings.gallery,
      source.gallery
    ),

    pressConferences: mergeSection(
    ...
    executiveCommittee: {
      ...
      media: normalizeMedia(source.executiveCommittee?.media),
    },

    officeBearers: {
      ...defaultHomeSettings.officeBearers,
      ...(source.officeBearers || {}),
      media: normalizeMedia(source.officeBearers?.media),
    },

    membership: {
      ...defaultHomeSettings.membership,
      ...(source.membership || {}),
      media: normalizeMedia(source.membership?.media),
      primaryHref:
        source.membership?.primaryHref ||
        defaultHomeSettings.membership.primaryHref,
    },
  };
}
