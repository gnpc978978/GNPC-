...
/**
 * Global public-site navbar.
 *
 * IMPORTANT:
 * - One consistent appearance on every public page.
 * - No scroll-based colour switching.
 * - The parent website layout keeps the whole header fixed.
 */
    ...
    <>
      <div className="w-full px-2 sm:px-4 lg:px-6">
        <nav
          ...
          className="relative z-[120] mx-auto w-full max-w-[1500px] overflow-hidden rounded-b-[1.35rem] border-x border-b border-slate-200 bg-white shadow-[0_12px_40px_rgba(15,23,42,0.12)] sm:rounded-b-[1.75rem]"
        >
          <div
            aria-hidden="true"
          ...
          <div className="relative flex min-h-[68px] items-center justify-between gap-3 px-3 sm:min-h-[76px] sm:px-5 lg:px-7">
            ...
            {/* ADMIN LOGIN — always high contrast */}
            <div className="hidden shrink-0 xl:block">
              <Link
                href="/admin/login"
                className="group/admin inline-flex h-11 items-center justify-center gap-2 rounded-full border border-[#0f4c81] bg-[#0f4c81] px-5 text-xs font-extrabold tracking-[0.08em] text-white shadow-[0_8px_25px_rgba(15,76,129,0.20)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#0b3d68] hover:shadow-[0_12px_30px_rgba(15,76,129,0.28)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f4c81] focus-visible:ring-offset-2"
              >
                <span>Admin Login</span>
                <HiArrowUpRight
                  size={16}
                  className="transition-transform duration-300 group-hover/admin:translate-x-0.5 group-hover/admin:-translate-y-0.5"
                  aria-hidden="true"
                />
              </Link>
            </div>
