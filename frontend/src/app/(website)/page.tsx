...
export default function Home() {
  ...
  useEffect(() => {
    const fetchLatestUpdates = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/api/latest-updates/public`);
        if (res.ok) {
    ...
    const fetchGallery = async () => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
        const res = await fetch(`${apiUrl}/api/gallery/public`);
        if (res.ok) {
          const result = await res.json();
          const items = Array.isArray(result)
            ? result
  ...
  const quickStats = [
    ...
    { label: "Press Releases", value: "200+", icon: Newspaper },
  ];
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Latest Updates Column (2 cols wide) */}
            <div className="lg:col-span-2 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" asChild size="sm">
                  <Link href="/latest-updates" className="flex items-center gap-1">
            ...
            {/* Event Calendar Sidebar (1 col wide) */}
          ...
          {loadingGallery ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {galleryItems.map((item) => {
                const imageSrc = item.imageUrl || item.url || "/placeholder.jpg";
                return (
                  <div
                    key={item._id}
